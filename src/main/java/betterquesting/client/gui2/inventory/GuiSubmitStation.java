...
public class GuiSubmitStation extends GuiContainerCanvas implements INeedsRefresh {
    ...
    public void refreshGui() {
        ...
        if (qc != null) {
            quests.addAll(
                QuestDatabase.INSTANCE.filterKeys(qc.getActiveQuests())
                    .entrySet());
        }
        filterQuests();
    ...
    public void initPanel() {
        ...
        btnSet = new PanelButton(new GuiTransform(new Vector4f(0.75F, 0F, 0.75F, 0F), -16, 64, 16, 16, 0), -1, "") {
            ...
            @Override
            public void onButtonClick() {
                final IQuest quest = quests.get(selQuest)
                    .getValue();
                final ITask task = tasks.get(selTask)
                    .getValue();
                tile.setupTask(QuestingAPI.getQuestingUUID(mc.thePlayer), quest, task);
                NetStationEdit.setupStation(
                    tile.xCoord,
                    tile.yCoord,
                    tile.zCoord,
                    QuestDatabase.INSTANCE.lookupKey(quest),
                    quest.getTasks()
                        .getID(task));
                refreshTaskPanel();
            }
        ...
        txtQstTitle = new PanelTextBox(
            ...
            "");
        txtQstTitle.setColor(PresetColor.TEXT_MAIN.getColor())
            .setAlignment(1);
        ...
        txtTskTitle = new PanelTextBox(
            ...
            "");
        txtTskTitle.setColor(PresetColor.TEXT_MAIN.getColor())
            .setAlignment(1);
    ...
    private void setInventoryPosition(int x, int y) {
        ...
        this.addPanel(
            new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_LEFT, x + 54, y, 18, 18, -1),
                PresetTexture.ITEM_FRAME.getTexture()));
        this.addPanel(
            new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_LEFT, x + 72, y, 18, 18, -1),
                PresetIcon.ICON_RIGHT.getTexture()));
        this.addPanel(
            new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_LEFT, x + 90, y, 18, 18, -1),
                PresetTexture.ITEM_FRAME.getTexture()));
    }
    ...
    private void filterQuests() {
        ...
        while (iter.hasNext()) {
            ...
            for (DBEntry<ITask> tsk : entry.getValue()
                .getTasks()
                .getEntries()) {
    ...
    private void refreshTaskPanel() {
        ...
        if (tile.isSetup()) {
            ...
            if (qdbe != null) {
                List<DBEntry<ITask>> tmpTasks = qdbe.getValue()
                    .getTasks()
                    .getEntries();
        ...
        txtTskTitle.setText(
            QuestTranslation.translate(
                curTask.getValue()
                    .getUnlocalisedName()));
        btnSet.setActive(
            !tile.isSetup() && (curTask.getValue() instanceof IItemTask || curTask.getValue() instanceof IFluidTask));
