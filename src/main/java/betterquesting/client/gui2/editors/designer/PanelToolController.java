...
public class PanelToolController implements IGuiPanel {
    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        if (activeTool != null) {
            ...
            // Pretending we're on the scrolling canvas (when we're really not) so as not to influence it by hotswapping
            // panels
    ...
    public boolean onMouseClick(int mx, int my, int button) {
        if (activeTool != null && this.getTransform()
            .contains(mx, my)) {
            ...
            if (activeTool.useSelection()) {
                if (button == 1) {
                    ...
                    int lsx = questLine.getScrollX();
                    int lsy = questLine.getScrollY();
                    int tx = questLine.getTransform()
                        .getX();
                    int ty = questLine.getTransform()
                        .getY();
                    int smx = (int) ((mx - tx) / zs) + lsx;
                    int smy = (int) ((my - ty) / zs) + lsy;
