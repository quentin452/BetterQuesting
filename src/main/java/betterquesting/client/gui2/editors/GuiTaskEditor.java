...
public class GuiTaskEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen, INeedsRefresh {
    ...
    private void onButtonPress(PEventButton event) {
        ...
        {
            ITask task = ((PanelButtonStorage<ITask>) btn).getStoredValue();
            GuiScreen editor = task.getTaskEditor(this, Maps.immutableEntry(qID, quest));
    ...
    private void refreshTasks() {
        List<DBEntry<ITask>> dbTsk = quest.getTasks()
            .getEntries();

        qtList.resetCanvas();
        int w = qtList.getTransform()
            .getWidth();

        for (int i = 0; i < dbTsk.size(); i++) {
            ITask task = dbTsk.get(i)
                .getValue();
            qtList.addPanel(
                new PanelButtonStorage<>(
