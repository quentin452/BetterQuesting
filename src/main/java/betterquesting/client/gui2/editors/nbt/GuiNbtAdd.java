...
public class GuiNbtAdd extends GuiScreenCanvas implements IPEventListener, IVolatileScreen {
    ...
    public void initPanel() {
        ...
        if (nbt.getId() == 10) // NBTTagCompound
        {
            ...
            PanelTextBox txKeyTitle = new PanelTextBox(
                new GuiTransform(GuiAlign.TOP_CENTER, -100, 36, 200, 12, 0),
                EnumChatFormatting.RED + QuestTranslation.translate("betterquesting.gui.no_key"));
            txKeyTitle.setColor(PresetColor.TEXT_MAIN.getColor());
            cvBackground.addPanel(txKeyTitle);

            flKey = new PanelTextField<>(
                new GuiTransform(GuiAlign.TOP_CENTER, -100, 48, 200, 16, 0),
                "",
            ...
            flKey.setCallback(value -> {
                if (value.isEmpty()) {
                    txKeyTitle
                        .setText(EnumChatFormatting.RED + QuestTranslation.translate("betterquesting.gui.no_key"));
                } else if (((NBTTagCompound) nbt).hasKey(value)) {
                    txKeyTitle.setText(
                        EnumChatFormatting.RED + QuestTranslation.translate("betterquesting.gui.duplicate_key"));
                } else {
        ...
        options.add(
            new PanelButtonStorage<>(
                new GuiTransform(GuiAlign.MID_CENTER, 0, n * 16, 100, 16, 0),
                2,
        ...
        options.add(
            new PanelButtonStorage<>(
                new GuiTransform(GuiAlign.MID_CENTER, 100, n++ * 16, 92, 16, 0),
                2,
        ...
        options.add(
            new PanelButtonStorage<>(
                new GuiTransform(GuiAlign.MID_CENTER, 0, n++ * 16, 192, 16, 0),
                2,
                QuestTranslation.translate("betterquesting.btn.entity"),
                JsonHelper.EntityToJson(new EntityPig(mc.theWorld), new NBTTagCompound())));
            ...
            case 1: // Confirm
            {
                if (selected == null) {
                    ...
                    if (index == l.tagCount()) {
                        ...
                        NBTConverter.getTagList((NBTTagList) nbt)
                            .add(index, selected);
                    }
                }
    ...
    private void updateConfirm() {
        ...
        } else if (flKey.getValue()
            .isEmpty() || ((NBTTagCompound) nbt).hasKey(flKey.getValue())) {
                btnConfirm.setActive(false);
            } else {
                btnConfirm.setActive(selected != null);
            }
