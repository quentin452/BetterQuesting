...
public class GuiEntitySelection extends GuiScreenCanvas implements IPEventListener, IVolatileScreen {
    ...
    public GuiEntitySelection(GuiScreen parent, NBTTagCompound tag, ICallback<Entity> callback) {
        this(parent, JsonHelper.JsonToEntity(tag, Minecraft.getMinecraft().theWorld), callback);
    }
    ...
    public void initPanel() {
        ...
        pnPreview = new PanelEntityPreview(
        ...
        if (btn.getButtonID() == 0) // Exit
        {
            try {
                ...
                QuestingAPI.getLogger()
                    .error("Unable to return entity selection!", e);
            }

            mc.displayGuiScreen(this.parent);
        } else if (btn.getButtonID() == 1 && btn instanceof PanelButtonStorage) {
            Entity e = EntityList
                .createEntityByName(((PanelButtonStorage<String>) btn).getStoredValue(), this.mc.theWorld);
