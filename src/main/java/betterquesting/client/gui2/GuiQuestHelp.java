...
public class GuiQuestHelp extends GuiScreenCanvas {
    ...
    public void initPanel() {
        ...
        PanelTextBox txtTitle = new PanelTextBox(
            new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(124, 36, 16, -48), 0),
            "").setAlignment(1);
        ...
        int width = cvTopics.getTransform()
            .getWidth();
        HelpTopic[] topics = HelpRegistry.INSTANCE.getTopics();
        for (int i = 0; i < topics.length; i++) {
            HelpTopic ht = topics[i];
            cvTopics.addPanel(
                new PanelButtonStorage<>(new GuiRectangle(0, i * 16, width, 16, 0), -1, ht.getTitle(), ht)
                    .setCallback(value -> {
