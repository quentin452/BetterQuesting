...
public class GuiPartyCreate extends GuiScreenCanvas implements IPEventListener, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        PanelPlayerPortrait pnPortrait = new PanelPlayerPortrait(
            new GuiTransform(GuiAlign.TOP_CENTER, -32, 0, 64, 64, 0),
            mc.thePlayer).setDepth(-16F);
        cvLeftHalf.addPanel(pnPortrait);

        cvLeftHalf.addPanel(
            new PanelGeneric(
                new GuiTransform(GuiAlign.TOP_CENTER, 16, 48, 24, 24, 0),
                new ItemTexture(
                    new BigItemStack(
                        BetterQuesting.extraLife,
                        LifeDatabase.INSTANCE.getLives(QuestingAPI.getQuestingUUID(mc.thePlayer))),
                    true,
                    true).setDepth(32F)));
        ...
        flName = new PanelTextField<>(
            new GuiTransform(GuiAlign.BOTTOM_EDGE, new GuiPadding(16, -32, 16, 16), 0),
            "New Party",
            FieldFilterString.INSTANCE);
        cvLeftHalf.addPanel(flName);

        PanelButton btnCreate = new PanelButton(
            new GuiTransform(GuiAlign.BOTTOM_EDGE, new GuiPadding(16, -16, 16, 0), 0),
            1,
    ...
    private void refreshInvites() {
        ...
        int cvWidth = invitePanel.getTransform()
            .getWidth();
        ...
        for (int i = 0; i < invites.size(); i++) {
            ...
            flExpiry.setFillColor(
                new GuiColorTransition(
        ...
        inviteScroll.setActive(
            invitePanel.getScrollBounds()
                .getHeight() > 0);
    }
}
