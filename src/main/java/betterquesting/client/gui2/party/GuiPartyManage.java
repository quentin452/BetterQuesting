...
public class GuiPartyManage extends GuiScreenCanvas implements IPEventListener, INeedsRefresh {
    ...
    public void refreshGui() {
        ...
        if (!flName.isFocused()) flName.setText(
            party.getProperties()
    ...
    public void initPanel() {
        ...
        PanelTextBox txTitle = new PanelTextBox(
            ...
            QuestTranslation.translate(
                ...
                party.getProperties()
                    .getProperty(NativeProps.NAME))).setAlignment(1);
        ...
        if (flName == null) flName = new PanelTextField<>(
        ...
        btnSetName.getTransform()
            .setParent(flName.getTransform());
        ...
        scUserList.getTransform()
            .setParent(cvUserList.getTransform());
        ...
        int cvWidth = cvUserList.getTransform()
            .getWidth();
        ...
        for (int i = 0; i < partyMemList.size(); i++) {
            ...
            PanelPlayerPortrait pnPortrait = new PanelPlayerPortrait(
                new GuiRectangle(0, i * 32, 32, 32, 0),
                mid,
                mName);
            ...
            PanelTextBox txLives = new PanelTextBox(
                new GuiRectangle(48, i * 32 + 20, cvWidth - 48 - 32, 12, 0),
                lifeCount);
            txLives.setColor(PresetColor.TEXT_MAIN.getColor());
            cvUserList.addPanel(txLives);
        }

        scUserList.setActive(
            cvUserList.getScrollBounds()
                .getHeight() > 0);
