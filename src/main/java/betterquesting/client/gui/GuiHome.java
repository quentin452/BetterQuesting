...
public class GuiHome extends GuiQuesting {
	private static GuiHome instance;
	private ResourceLocation homeGui = new ResourceLocation(BQ_Settings.titleCard);

	private GuiHome(GuiScreen parent) {
		super(parent, "");
	}

	public static GuiHome getInstance() {
		if (instance == null) {
			instance = new GuiHome(Minecraft.getMinecraft().currentScreen);
		}
		return instance;
	}
	...
	public void actionPerformed(GuiButton button) {
		...
		if (button.id == 1) {
			switchScreen(GuiQuestLinesMain.getInstance());
		} else if (button.id == 2) {
			PartyInstance party = PartyManager.GetParty(mc.thePlayer.getUniqueID());

			if (party != null) {
				switchScreen(new GuiManageParty(this, party));
			} else {
				switchScreen(new GuiNoParty(this));
			}
		} else if (button.id == 3) {
			switchScreen(new GuiThemeSelect(this));
		}
	}
}
