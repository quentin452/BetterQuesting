...
public class GuiQuestLinesMain extends GuiQuesting {
	private static GuiQuestLinesMain instance;
	...
	private GuiQuestLinesMain(GuiScreen parent) {
		super(parent, I18n.format("betterquesting.title.quest_lines"));
	}

	public static GuiQuestLinesMain getInstance() {
		if (instance == null) {
			instance = new GuiQuestLinesMain(GuiHome.getInstance());
		}
		return instance;
	}
	...
	public void drawScreen(int mx, int my, float partialTick) {
		...
		this.drawTexturedModalRect(guiLeft + 16 + 142, this.guiTop + 32 + (int) Math.max(0, i * (float) listScroll / (qlBtns.size() - maxRows)), 248, 60, 8, 20);
	...
	public void actionPerformed(GuiButton button) {
		...
		if (button.id == 1) {
			switchScreen(new GuiQuestLineEditorA(this));
			// Quest line editor
	...
	protected void mouseClicked(int mx, int my, int type) {
		...
		if (qlGui != null && type == 0) {
			...
			if (qBtn != null) {
				...
				switchScreen(new GuiQuestInstance(this, qBtn.quest));
			}
		}
	}
