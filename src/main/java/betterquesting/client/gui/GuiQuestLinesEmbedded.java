	...
	public void drawGui(int mx, int my, float partialTick) {
		...
		if (qLine != null) {
			...
			for (GuiButtonQuestInstance btnQuest : qBtns) {
				...
				if (btnQuest.visible && screen.isWithin(rmx, rmy, btnQuest.xPosition + scrollX, btnQuest.yPosition + scrollY, btnQuest.width, btnQuest.height, false)
						&& screen.isWithin(mx, my, posX, posY, sizeX, sizeY, false)) {
			...
			screen.mc.fontRenderer.drawString(I18n.format(qLine.name), MathHelper.ceiling_float_int((posX + 4) / scale), MathHelper.ceiling_float_int((posY + 4) / scale),
					ThemeRegistry.curTheme().textColor().getRGB(), false);
			screen.mc.fontRenderer.drawString(zoom + "%", MathHelper.ceiling_float_int((posX + 4) / scale),
					MathHelper.ceiling_float_int((posY + sizeY - 4 - screen.mc.fontRenderer.FONT_HEIGHT) / scale), ThemeRegistry.curTheme().textColor().getRGB(), false);
			GL11.glPopMatrix();
		}
