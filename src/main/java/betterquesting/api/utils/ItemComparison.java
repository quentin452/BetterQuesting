...
public class ItemComparison {
    ...
    public static boolean StackMatch(ItemStack stack1, ItemStack stack2, boolean nbtCheck, boolean partialNBT) {
        ...
        if (!(stack1.getItemDamage() == stack2.getItemDamage() || stack1.getItem()
            .isDamageable() || stack1.getItemDamage() == OreDictionary.WILDCARD_VALUE)) {
            return false;
        }
        if (nbtCheck) {
    ...
    public static boolean CompareNBTTag(NBTBase tag1, NBTBase tag2, boolean partial) {
        ...
        if (tag1 instanceof NBTTagCompound && tag2 instanceof NBTTagCompound) {
            ...
            if (list1.func_150302_c().length > list2.func_150302_c().length
                || (!partial && list1.func_150302_c().length != list2.func_150302_c().length)) {
                return false; // Sample is missing requested tags or is not exact
            }

            List<Integer> usedIdxs = new ArrayList<>(list2.func_150302_c().length); // Duplicate control

            topLoop: for (int i = 0; i < list1.func_150302_c().length; i++) {
            ...
            if (list1.func_150292_c().length > list2.func_150292_c().length
                || (!partial && list1.func_150292_c().length != list2.func_150292_c().length)) {
            ...
            topLoop: for (int i = 0; i < list1.func_150292_c().length; i++) {
        ...
        } else if (tag1 instanceof NBTPrimitive && tag2 instanceof NBTPrimitive) // Standardize numbers to not care
                                                                                 // about format
        {
    ...
    public static boolean AllMatch(ItemStack stack1, ItemStack stack2, boolean nbtCheck, boolean partialNBT) {
        ...
        if (StackMatch(stack1, stack2, false, false)) return true; // Stacks are compatible (NBT was already checked at
                                                                   // this point)
