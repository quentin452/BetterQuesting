...
public class QuestCommandComplete extends QuestCommandBase {
	...
	public void runCommand(CommandBase command, ICommandSender sender, String[] args) {
		...
		try {
			...
			quest.UpdatePartyMembers(uuid);
			if (player != null) {
				player.addChatMessage(new ChatComponentTranslation("betterquesting.cmd.complete", new ChatComponentTranslation(quest.name), player.getCommandSenderName()));
			}
		} catch (Exception e) {
			throw getException(command);
		}
