...
public class PartyInvitations implements INBTPartial<NBTTagList, UUID> {
    ...
    public synchronized void purgeInvites(int partyID) {
        invites.values()
            .forEach((value) -> value.remove(partyID));
    }

    public synchronized void cleanExpired() {
        MinecraftServer server = MinecraftServer.getServer();
        Iterator<Entry<UUID, HashMap<Integer, Long>>> iterA = invites.entrySet()
            .iterator();
        while (iterA.hasNext()) {
            Entry<UUID, HashMap<Integer, Long>> userInvites = iterA.next();

            List<Integer> revoked = new ArrayList<>();
            Iterator<Entry<Integer, Long>> iterB = userInvites.getValue()
                .entrySet()
                .iterator();
            while (iterB.hasNext()) {
            ...
            if (player != null && revoked.size() >= 0) {
                ...
                NetInviteSync.sendRevoked(player, revAry); // Normally I avoid including networking calls into the
                                                           // database...
            }
            if (userInvites.getValue()
                .size() <= 0) iterA.remove();
        }
    }
    ...
    @Override
    public synchronized NBTTagList writeToNBT(NBTTagList nbt, @Nullable List<UUID> subset) // Don't bother saving this
                                                                                           // to disk. We do need to
                                                                                           // send packets though
    {
        if (subset != null) {
            subset.forEach((uuid) -> {
            ...
            for (Entry<UUID, HashMap<Integer, Long>> userMap : invites.entrySet()) {
                ...
                NBTTagList invList = new NBTTagList();
                for (Entry<Integer, Long> invEntry : userMap.getValue()
                    .entrySet()) {
                    NBTTagCompound invTag = new NBTTagCompound();
                    invTag.setInteger("partyID", invEntry.getKey());
                    invTag.setLong("expiry", invEntry.getValue());
