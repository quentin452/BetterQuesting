 ...
 * Useful if your panel/canvas is contained within a screen you're not in control of but still needs to respond to
 * events
 */
public class PEventBroadcaster {
    ...
    public void unregister(@Nonnull Consumer<PanelEvent> consumer) {
        entryList.values()
            .forEach((value) -> value.unregisterListener(consumer));
    }
    ...
    public boolean postEvent(@Nonnull PanelEvent event) {
        ...
        for (Entry<Class<? extends PanelEvent>, PEventEntry<? extends PanelEvent>> e : entryList.entrySet()) {
            if (!e.getKey()
                .isAssignableFrom(event.getClass())) continue;
            e.getValue()
                .fire(event);
        }
    ...
    /**
     * Clears event listeners whenever a new GUI loads. If you must have cross GUI communication either handle this
     * yourself or re-register the relevant listeners.
     */
    @SubscribeEvent
