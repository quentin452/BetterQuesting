...
public class PanelEntityPreview implements IGuiPanel {
    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        float pitch;
        if (EntityList.getEntityString(entity)
            .contains("Wisp")) {
            changeTheCameraPitch(90F);
            pitch = 90F;
        } else if (EntityList.getEntityString(entity)
            .equals("TwilightForest.Naga")) {
                pitch = pitchDriver.readValue();
                scale /= 2;
            } else pitch = pitchDriver.readValue();
        ...
        if (thePlayerPitch != Minecraft.getMinecraft().thePlayer.rotationPitch) changeTheCameraPitch(thePlayerPitch);
        GL11.glColor4f(1F, 1F, 1F, 1F);
    }
