...
public class PanelItemSlot extends PanelButtonStorage<BigItemStack> {
    ...
    public BigItemStack getStoredValue() {
        if (!oreVariants.isEmpty()) {
            return oreVariants.get(
                (int) Math.floor((System.currentTimeMillis() / 1000D) % (oreVariants.size() * interval) / interval));
        }
    ...
    public PanelItemSlot setStoredValue(BigItemStack value) {
        ...
        if (value != null) {
            ...
            this.setIcon(
                oreDict || value.getBaseStack()
                    .getItemDamage() == OreDictionary.WILDCARD_VALUE
                        ? new OreDictTexture(interval, value, showCount, true)
                        : new ItemTexture(value, showCount, true),
                1);
            try {
                this.setTooltip(
                    value.getBaseStack()
                        .getTooltip(mc.thePlayer, mc.gameSettings.advancedItemTooltips));
                ...
                this.setTooltip(
                    Collections.singletonList(
                        "" + EnumChatFormatting.RED
                            ...
                            + "Broken tooltip. Please REPORT TO DEV."));
            }
        } else {
            this.setIcon(null);
    ...
    private void updateOreStacks() {
        ...
        for (ItemStack iStack : stack.getOreIngredient()
            .getMatchingStacks()) {
            if (iStack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                List<ItemStack> subItems = new ArrayList<>();
                iStack.getItem()
                    .getSubItems(iStack.getItem(), CreativeTabs.tabAllSearch, subItems);
