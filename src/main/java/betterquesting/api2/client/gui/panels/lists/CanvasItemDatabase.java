...
public class CanvasItemDatabase extends CanvasSearch<ItemStack, Item> {
    ...
    protected void queryMatches(Item item, String query, final ArrayDeque<ItemStack> results) {
        ...
        String regName = Item.itemRegistry.getNameForObject(item); // This is gonna be really slow but... meh. It's rate
                                                                   // limited in the parent class
        ...
        try {
            ...
            if (regName.toLowerCase()
                .contains(query)
                || item.getUnlocalizedName()
                    .toLowerCase()
                    .contains(query)
                || QuestTranslation.translate(item.getUnlocalizedName())
                    .toLowerCase()
                    .contains(query)) {
                    ...
                    try {
                        if (subItem.getUnlocalizedName()
                            ...
                            || subItem.getDisplayName()
                                ...
                                .contains(query)) {
                            results.add(subItem);
                        } else if (Arrays.stream(OreDictionary.getOreIDs(subItem))
                            .anyMatch(
                                (id) -> OreDictionary.getOreName(id)
                                    .toLowerCase()
                                    .contains(query))) {
                                        results.add(subItem);
                                    }
                        ...
                        BetterQuesting.logger.error(
                            "An error occured while searching itemstack " + subItem.toString()
                                + " from item \""
                                + regName
                                + "\" ("
                                + item.getClass()
                                    .getName()
                                + ").\nNBT: "
                                + subItem.writeToNBT(new NBTTagCompound()),
                            e);
                    }
                });
        } catch (Exception e) {
            BetterQuesting.logger.error(
                "An error occured while searching item \"" + regName
                    + "\" ("
                    + item.getClass()
                        .getName()
                    + ")",
