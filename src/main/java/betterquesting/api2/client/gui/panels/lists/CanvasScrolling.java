...
public class CanvasScrolling implements IGuiCanvas {
    ...
    public CanvasScrolling(IGuiRect rect) {
        ...
        zoomScale = new FloatSimpleIO(1F, 0.2F, 2F)
            .setLerp(BQ_Settings.zoomTimeInMs > 0, BQ_Settings.zoomTimeInMs > 0 ? 1.0F / BQ_Settings.zoomTimeInMs : 0);
    }
    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        if (isDragging && (Mouse.isButtonDown(0) || Mouse.isButtonDown(2))) // Extra fallback incase something used the
                                                                            // usual release event
        {
        ...
        if (lsz != zs) {
            ...
            if ((zoomIn && !BQ_Settings.zoomInToCursor) || (!zoomIn && !BQ_Settings.zoomOutToCursor)) {
                float change = zs / lsz; // This could probably crash if someone allowed the zoom driver to go to zero.
                                         // Not really my fault to be fair
                ...
                this.refreshScrollBounds(); // NOTE: This runs updatePanelPcroll() too. Hence why the math above is done
                                            // first before the scroll bounds are changed
                ...
                this.refreshScrollBounds(); // NOTE: This runs updatePanelScroll() too. Hence why the math above is done
                                            // first before the scroll bounds are changed
    ...
    public boolean onMouseScroll(int mx, int my, int scroll) {
        ...
        if (!used) {
            if (zoomMode) {
                ...
                if (scroll > 0) zoomScale.writeValue(cs / zoomSpeed);
                else if (scroll < 0) zoomScale.writeValue(cs * zoomSpeed);
    ...
    public void refreshScrollBounds() {
        ...
        for (IGuiPanel panel : guiPanels) {
            if (first) {
                left = panel.getTransform()
                    .getX();
                top = panel.getTransform()
                    .getY();
                right = panel.getTransform()
                    .getX()
                    + panel.getTransform()
                        .getWidth();
                bottom = panel.getTransform()
                    .getY()
                    + panel.getTransform()
                        .getHeight();
                first = false;
