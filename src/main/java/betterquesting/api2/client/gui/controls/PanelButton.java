    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        if (texIcon != null) {
            ...
            if (isz > 0) {
                if (colIcon != null) {
                    texIcon.drawTexture(
                        bounds.getX() + (bounds.getWidth() / 2) - (isz / 2),
    ...
    private static void drawCenteredString(FontRenderer font, String text, int x, int y, int width, int color,
        ...
        switch (align) {
            ...
            case 2: {
                font.drawString(
                    text,
                    MathHelper.floor_float(x + width - RenderUtils.getStringWidth(text, font) / 2F - 4),
                    y,
                    color,
                    shadow);
                break;
            ...
            default: {
                font.drawString(
                    text,
                    MathHelper.floor_float(x + Math.floorDiv(width, 2) - RenderUtils.getStringWidth(text, font) / 2F),
                    y,
                    color,
                    shadow);
            }
