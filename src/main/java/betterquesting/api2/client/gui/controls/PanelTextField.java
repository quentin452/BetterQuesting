...
public class PanelTextField<T> implements IGuiPanel {
    ...
    // Yep... we're supporting this without a scrolling canvas (we don't need the zooming and mouse dragging but the
    // scrolling bounds change much more often)
    ...
    public PanelTextField(IGuiRect rect, String text, IFieldFilter<T> filter) {
        ...
        this.setTextures(
            PresetTexture.TEXT_BOX_0.getTexture(),
            PresetTexture.TEXT_BOX_1.getTexture(),
    ...
    public void setScrollX(int value) {
        ...
        scrollX.writeValue(MathHelper.clamp_int(value, 0, scrollWidth) / (float) scrollWidth);
    }
    ...
    public void setScrollY(int value) {
        ...
        scrollY.writeValue(MathHelper.clamp_int(value, 0, scrollHeight) / (float) scrollHeight);
    }
    ...
    /**
     * Deletes the given number of whole words from the current cursor's position, unless there is currently a
     * selection, in
    ...
    public boolean onKeyTyped(char typedChar, int keyCode) {
        ...
        } else if (keyCode == 30 && isKeyComboCtrl()) // Ctrl + A
        {
            ...
            return true;
        } else if (keyCode == 46 && isKeyComboCtrl()) // Ctrl + C
        {
            GuiScreen.setClipboardString(this.getSelectedText());
            return true;
        } else if (keyCode == 47 && isKeyComboCtrl()) // Ctrl + V
        {
            if (this.isActive) {
            ...
            return true;
        } else if (keyCode == 45 && isKeyComboCtrl()) // Ctrl + X
        {
            GuiScreen.setClipboardString(this.getSelectedText());
    ...
    public void drawPanel(int mx, int my, float partialTick) {
        ...
        GL11.glTranslatef(-getScrollX(), -getScrollY(), 0);

        if (text.length() <= 0) {
            if (!isFocused) {
                mc.fontRenderer
                    .drawString(watermark, bounds.getX() + 4, bounds.getY() + 4, colWatermark.getRGB(), false);
            }
        } else {
