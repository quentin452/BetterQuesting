...
public class PanelTextField<T> implements IGuiPanel
{
    ...
    {
        return GuiScreen.isCtrlKeyDown() && !GuiScreen.isShiftKeyDown() && !(Keyboard.isKeyDown(56) || Keyboard.isKeyDown(184));
    }
    ...
    public void drawPanel(int mx, int my, float partialTick)
    {
        ...
        if(isActive && dragging && Mouse.isButtonDown(0))
        {
            if(canWrap)
            {
                setSelectionPos(RenderUtils.getCursorPos(text, mx - (transform.getX() + 4) + getScrollX(), my - (transform.getY() + 4) + getScrollY(), transform.getWidth() - 8, mc.fontRenderer));
            } else
            {
                setSelectionPos(RenderUtils.getCursorPos(text, mx - (transform.getX() + 4) + getScrollX(), mc.fontRenderer));
            }
        } else if(dragging)
        {
            dragging = false;
