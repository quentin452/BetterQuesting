...
public class FluidTexture implements IGuiTexture {
    ...
    public void drawTexture(int x, int y, int width, int height, float zDepth, float partialTick, IGuiColor color) {
        ...
        IIcon icon = fluid.getFluid()
            .getIcon() != null ? fluid.getFluid()
                .getIcon()
                : ((TextureMap) mc.renderEngine.getTexture(TextureMap.locationBlocksTexture))
                    .getAtlasSprite("missigno");
    ...
    private void drawTexturedModalRect(double xCoord, double yCoord, double zDepth, IIcon textureSprite, double widthIn,
        double heightIn) {
        Tessellator tessellator = Tessellator.instance;
        tessellator.startDrawingQuads();
        tessellator
            .addVertexWithUV(xCoord, yCoord + heightIn, zDepth, textureSprite.getMinU(), textureSprite.getMaxV());
