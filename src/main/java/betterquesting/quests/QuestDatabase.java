...
public class QuestDatabase {
	...
	/**
	 * Allows quests to be edited by moderators. Once disabled it can only be re-enabled via editing the JSON file. Use this to lock down the quest lines before
	 * publishing them. This can safely be left on if moderators are required to edit/maintain quests during use, normal users will have the buttons disabled
	 * and any forced edits ignored.
	 */
    ...
    */
	...
	public static ArrayList<QuestInstance> getActiveQuests(UUID uuid) {
		ArrayList<QuestInstance> questList = new ArrayList<>();
	...
	public static ArrayList<TaskBase> getActiveTasks(UUID uuid) {
		ArrayList<TaskBase> taskList = new ArrayList<>();
	...
	public static void DeleteQuest(int id) {
		...
		for (QuestLine line : questLines) {
			for (Iterator<QuestLineEntry> it = line.questList.iterator(); it.hasNext(); ) {
				QuestLineEntry e = it.next();
				if (e.quest.questID == id) {
					it.remove();
				}
			}
		}
	...
	public static void sendDBToParty(UUID uuid) {
		PartyInstance party = PartyManager.GetParty(uuid);

		if (party == null) {
			SendDatabase(PacketAssembly.getPlayerForUUID(uuid));
		} else {
			for (PartyMember mem : party.GetMembers()) {
				SendDatabase(PacketAssembly.getPlayerForUUID(mem.userID));
			}
		}
	}
	...
	public static void readFromJson_keepProgression(JsonObject json) {
	...
	public static void writeToJson_Progression(JsonObject json) {
		...
		for (QuestInstance quest : questDB.values()) {
			JsonObject questJson = new JsonObject();
			quest.writeProgressToJSON(questJson, null);
			dbJson.add(questJson);
		}
